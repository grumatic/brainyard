...
| CR-RSN-13 | `:sandbox` MUST run in the SCI sandbox: `def` persists across iterations and turns within a session, output is captured, errors are non-fatal, **no interop** (System/Runtime/ProcessBuilder/ClassLoader denied), with auto-background detach at `:auto-background-timeout-ms` (default 30s). | Implemented | `execution-model-sandbox`, `coact_agent.clj` |
