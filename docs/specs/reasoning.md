...
| CR-RSN-14 | `:nrepl` MUST run in the live JVM: full interop/reflection and a pinned session via `:nrepl-session-id`. It is a full-trust backend — the only eval gate is a deny-list of catastrophic substrings; the earlier read-only/mutate grant, scope, confirmation, audit, and drift layers were **removed**. | Implemented (deny-list only) | `execution-model-nrepl`, `agent-nrepl-session-id`, `coact_agent.clj`; deny-list in `clj-nrepl/core/client.clj` |
