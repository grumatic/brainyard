...
| ID | Contract | Status | Source |
|---|---|---|---|
| CR-TUI-12a | Slash commands MUST be dispatched by `handle-input-line`, covering at minimum: `/quit /status /history /clear /compact /todo /usage /display-format /model /config /feature /init /effort /help /continue /task /allow-path /capture /copy /sandbox /mcp /login /logout /agent /session /memory /pause /queue` (plus Mode-B `/activity /log /scrollback /popup`). `/continue` covers both ways a run stops — unparking one parked by `/pause`, and re-asking one that exhausted its iteration budget; the former `/resume` is gone. `/session` carries the persisted-session subcommands `tree /  fork / resume / rename` (formerly the standalone `/tree`, `/fork`, `/resume-session`, `/name`). | Implemented | `bases/agent-tui/.../commands.clj` (`handle-input-line`) |
...
| CR-TUI-12c | The command list MUST have exactly one source: `agent.tui.format/command-registry` (`[cmd args description completions?]`), from which both `format-help` and the autocomplete menu are derived — so `/help` and the completion menu cannot disagree with each other or with what dispatches. | Implemented | `components/agent/.../tui/format.clj` (`command-registry`, `format-help`); `bases/agent-tui/.../autocomplete.clj` |
| CR-TUI-12d | `/copy` MUST copy the **source** answer rather than the rendered screen (a cell selection picks up borders, indents, rails and our own hard wraps), stashing the raw answer per session as it is emitted — including when the box is suppressed. Mechanism order MUST be tmux → OSC 52 when `SSH_TTY` indicates a remote session → native (`pbcopy`/`wl-copy`/`xclip`/`clip.exe`) → OSC 52, because a native tool over SSH would succeed against the wrong machine. An OSC 52 send MUST NOT claim success (no reply channel). Empty and oversized payloads MUST be refused — an empty payload *clears* the clipboard, and past the control-string cap terminals truncate rather than error. | Implemented | `bases/agent-tui/.../clipboard.clj`; `session.clj` (answer stash) |
| CR-TUI-13 | Permission prompts MUST cache by session dir, use a Mode-B tmux popup when feasible, fall back to an in-stream raw-mode y/n/a prompt, and auto-deny with an `/allow-path` hint when non-raw. The wait is `:permission-timeout-ms` (default 60000) — **one** timeout for every prompt, ACP's `session/request_permission` included. | Implemented | `bases/agent-tui/.../permissions.clj` (`make-permission-fn`); `agent.core.config` (`:permission-timeout-ms`) |
...
| CR-TUI-17 | The on-disk store MUST live at `<project>/.brainyard/sessions/<id>/` (project-scoped) with canonical filenames for meta/session/messages/scrollback/layout/dialogs/permissions/queue/todo/status/input-history and a `by-host.lock`. | Implemented | `components/agent-tui-persist/.../paths.clj` |
