...
| CR-MEM-06 | A capture pipeline MUST subscribe to the agent lifecycle hooks (`:agent.ask/post`, `:agent.tool-use/post`, `:agent.code-eval/post`, `:agent/exception`), parse them (S1), and write recall-worthy episodes to L2. Capture policy: the conversation is **one Q&A episode at `:agent.ask/post`** (the bare `:agent.ask/pre` question is NOT captured — it self-recalled and doubled episode count) with a content-addressable `entry_id` so re-asks upsert; tool-use / code-eval are captured **only on error** (successes live in the trajectory log + `memory_audit`, queryable on demand via the `trajectory$search` command — the operational counterpart to semantic `memory$recall`). | Implemented | `memory/core/capture/{dispatcher,parser,sidecar}.clj`, `episodic.clj` (upsert), `sqlite.clj` (`memory_audit`) |
