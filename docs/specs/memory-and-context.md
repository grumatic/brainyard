...
| CR-MEM-05 | Ranking MUST use BM25 within a layer and Reciprocal Rank Fusion (RRF, default k=60) across layers. Default layer weights order the **distilled hierarchy** `{:l1 0.25 :l2 0.35 :l3 0.7 :vec 0.6 :graph 0.55}` — L3 (distilled community summaries) outranks the raw L2 episodes it was distilled from (~2×); L2 is the recency/empty-L3 fallback; L1 is lowest (already verbatim in the prompt). RRF is rank-based, so the high L3 weight is free when L3 is empty. | Implemented | `memory/core/recall.clj` (`reciprocal-rank-fusion`), `recall_v2.clj` (`default-weights`, `recall-layered`) |
| CR-MEM-06 | A capture pipeline MUST subscribe to the agent lifecycle hooks and write recall-worthy episodes to L2. Capture policy (as-shipped): **only `:agent.ask/post` is subscribed** — the conversation is **one Q&A episode** per turn (the bare `:agent.ask/pre` question is NOT captured — it self-recalled and doubled episode count) with a content-addressable `entry_id` so re-asks upsert. **Only the USER's turns are captured** — the session root and any session-sharing subagent (an ACP connection is the user addressing a second model in their own session); a DISPATCHED subagent's ask/post is operational sub-task detail, dropped via the dispatcher `:match` predicate. Tool-use / code-eval / exception episodes are **no longer captured** — successes and errors both live in the trajectory log + `memory_audit`, queryable on demand via `trajectory$search` (the operational counterpart to semantic `memory$recall`). | Implemented | `memory/core/capture/{dispatcher,parser,sidecar}.clj`, `episodic.clj` (upsert), `agent/core/agent.clj` (`agent-capture-event?`), `sqlite.clj` (`memory_audit`) |
...
| CR-MEM-21 | A `sqlite-vec` `graph_vec` index + a `read-vec` recall producer MUST add semantic-similarity candidates to the RRF, via an injected `embed-fn`. Two providers: clj-llm `/embeddings` (any OpenAI-compatible endpoint) and a self-contained in-binary **Model2Vec** static embedder (`:graph-embed-model "static"`, pure-JVM, no server). Both extension + model fetched (sha-pinned) and bundled into the native image. No-op when neither configured. | Implemented (Phase 1 + 1.5) | `memory/core/embed.clj`, `embed_static.clj` (Model2Vec), `graph.clj` (`vec-search`), `sqlite.clj` (vec loader + dim-coupled `graph_vec`), `recall_v2.clj` |
| CR-MEM-22 | An LLM extraction pass MUST populate nodes/edges (entity + relationship extraction, name/alias resolution, functional-relation bi-temporal supersession, provenance), gated by an injected `extract-fn` (`:graph-extract-model`; off when unset). Two modes via `:graph-extract-mode` (default **`:at-consolidation`**): batch-extract the episodes captured since the last consolidation in ONE call at each consolidation (fewer LLM calls; the async per-episode extractor stays off); or `:per-episode` — one async extraction per captured L2 turn. Per-extraction input is bounded by `:graph-extract-max-input-chars` (400K ≈ 100K tokens); node/edge growth is confined by per-episode caps + a total node budget (`:graph-max-nodes` 100, lowest-retention eviction). | Implemented (Phase 2 + Design A) | `memory/core/extract.clj`, `capture/extractor.clj`, `capture/sidecar.clj` (`:on-write`), `interface.clj` (`extract-l2-batch!`, `start-capture! :defer-extraction?`), `memory-agent/hooks.clj` (batch at consolidation), `agent/core/memory.clj` (`graph-provider-opts`) |
