...
- **Consolidation cadence** — `:agent.ask/post`, id `::consolidation-cadence`. Increments a per-session turn counter (a plain atom — *no LLM*) and, every `:memory-consolidate-every-n-turns` turns (default 12), fire-and-forget runs the memory pipeline's L2→L3 reducer over the session: `mem/consolidate-graph!` (community / CR-MEM-24) when `:enable-graph-memory` is on, else the LLM-free heuristic `mem/consolidate-l2!`. **rev 4 (Design A):** in the default `:graph-extract-mode :at-consolidation`, the graph path first **batch-extracts** the episodes captured since the last consolidation (`mem/extract-l2-batch!`, advancing a per-session `!extract-marker` max-id so edges aren't re-inserted) *before* summarizing — the async per-episode extractor stays off.
- **Session-end flush** — `:agent.instance/closed`, id `::session-end-flush`. When a root agent instance closes (`/quit`, EOF, `/agent close`), runs one final reduce (incl. the batch-extract of the tail) so a session ending between cadence boundaries still promotes its tail of episodes. Fires *synchronously* (the event fires before `mem/stop-capture!`, so the manager is still live) but is bounded by a 10s timeout so a slow community/LLM reduce can't wedge shutdown. Clears the session's turn counter and extraction marker (the latter *after* the flush reduce reads it).
