...
- **rev 4 (2026-07-02)** — **capture/recall/consolidation rebalance + Design A extraction.** Five coupled changes: (1) **L2 capture is scoped to the user's turns** — a DISPATCHED subagent's `:agent.ask/post` is dropped via the dispatcher `:match` predicate (`agent.core.agent/agent-capture-event?`); those turns are operational sub-task detail. (Session-sharing subagents — ACP connections — ARE captured: their turns are the user's. The predicate was named `root-agent-capture-event?` until 2026-08-02, which misdescribed it.) (2) **Error/tool/eval episodes dropped** — capture now subscribes to **`:agent.ask/post` only** (no `tool-use/post`, `code-eval/post`, `exception`); those live in the trajectory log. (3) **L2 recall is cross-session** — `read-l2` passes `:exclude-session-id`, surfacing prior-session episodes (the current session's turns are already in previous-turns). (4) **Consolidation implied by `:enable-graph-memory`** — `consolidation-eligible?` returns true when *either* `:enable-memory-consolidation` OR `:enable-graph-memory` is on (the graph extractor builds communities that consolidation must harvest). (5) **Design A extraction** — `:graph-extract-mode :at-consolidation` (default) batch-extracts new episodes at each consolidation in one LLM call (`extract-l2-batch!`, incremental id marker in `hooks.clj`) instead of one call per turn; `:graph-extract-max-input-chars` 12000 → 400000. Recall defaults rebalanced to the distilled hierarchy `{:l1 0.25 :l2 0.35 :l3 0.7 :vec 0.6 :graph 0.55}`. `:memory-consolidate-every-n-turns` lowered 12 → **5**.
...
1. **Capture is automatic but indiscriminate.** *(rev-1 framing; capture was since narrowed — see rev 4: only `:agent.ask/post`, scoped to the user's turns, is captured; tool-use/code-eval/exception episodes were dropped.)* At the time, every `:agent.ask/post`, `:agent.tool-use/post`, and `:agent.code-eval/post` wrote an L2 episode through the S1 parser. The S2 reducer is a deterministic-prefix heuristic. There is no LLM in the loop deciding *what is actually worth remembering after this turn*. The chronicle grows linearly with iteration count; signal-to-noise drops as sessions get long.
...
### 10.0 Batch consolidation (rev 3, extended rev 4 — current)

Two deterministic hooks, gated by `consolidation-eligible?` — root-only, never memory-agent itself, and eligible when **either** `:enable-memory-consolidation` (default false) **OR** `:enable-graph-memory` is on (rev 4: the graph extractor builds communities that consolidation must harvest, so graph-on implies consolidation — derived at the read site, not a second config default):

- **Consolidation cadence** — `:agent.ask/post`, id `::consolidation-cadence`. Increments a per-session turn counter (a plain atom — *no LLM*) and, every `:memory-consolidate-every-n-turns` turns (default 5), runs the memory pipeline's L2→L3 reducer over the session **off the turn thread as a background `:fn` task** (`agent.common.background`, `:kind :consolidate` keyed by session — see self-improve-design.md §7 for the shared mechanism): `mem/consolidate-graph!` (community / CR-MEM-24) when `:enable-graph-memory` is on, else the LLM-free heuristic `mem/consolidate-l2!`. **rev 4 (Design A):** in the default `:graph-extract-mode :at-consolidation`, the graph path first **batch-extracts** the episodes captured since the last consolidation (`mem/extract-l2-batch!`, advancing a per-session `!extract-marker` max-id so edges aren't re-inserted) *before* summarizing — the async per-episode extractor stays off. **Single-flight per session:** a graph-mode reduce can outlast the next cadence boundary, so a boundary reached while one is still running is *dropped* rather than run concurrently — two overlapping reduces over one session's L2→L3 would race the `!extract-marker` and re-insert edges. Nothing is lost by the drop: the next boundary reduces whatever accumulated. **Offloaded in graph mode:** when a detached launcher is installed (`!offload-fn`, the same seam the session-end flush uses), a graph-mode cadence boundary spawns a detached `by memory reduce` child instead of running in-process — a 261s reduce is cancelled outright if the user quits, and a child outlives the session. Safe to share that seam only because the batch-extraction watermark is now PERSISTED (`graph_extract_after_id:…`, previously an in-process atom), so parent and child resume from one mark instead of re-extracting each other's episodes. Detached spawns get their own single-flight check (`detached-cadence-running?`, by pid) since the task-manager guard cannot see a child process. The heuristic path stays in-process — it is LLM-free and returns in milliseconds, so there is nothing to outlive. **Mode-aware job ceiling** for the in-process path (`cadence-job-timeout-ms`): the heuristic reduce is LLM-free and returns in milliseconds, but a live graph reduce over an 88-episode window took **261s** — 87% of the background layer's generic 300s default — so graph mode gets 30 min. The ceiling reclaims a wedged job; it is not a bound on normal work, and a cadence job cancelled mid-reduce could leave the extract-marker advanced with communities unsummarized.
- **Session-end flush** — `:agent.instance/closed`, id `::session-end-flush`. When a root agent instance closes (`/quit`, EOF, `/agent close`), runs one final reduce (incl. the batch-extract of the tail) so a session ending between cadence boundaries still promotes its tail of episodes. Fires *synchronously* (the event fires before `mem/stop-capture!`, so the manager is still live) but is bounded by a **mode-aware timeout** so a slow reduce can't wedge shutdown: 10s on the LLM-free heuristic path, **300s on the graph path** (batch extraction + per-community LLM summaries can take minutes on a long session — a short bound would abandon it and lose the tail). Because this blocks `/quit` inside `stop!` *before* the terminal is restored, the TUI emits a "⏳ Finalizing memory…" banner first (gated by `agent/pending-consolidation?`) so the pause isn't a mystery. Clears the session's turn counter and extraction marker (the latter *after* the flush reduce reads it).
