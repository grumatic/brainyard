...
>
> **As-built (2026-06):** update-agent is alive and unchanged in role — it is still the safe single-file-edit specialist that exec-agent/research-agent delegate writes to. The full `update$*` helper family (`update$slug`, `update$frontmatter`, `update$write`, `update$index-append`, `update$read-record`, `update$find`, and the `update$apply` mega-helper) ships in `common/update.clj` as `defcommand`s, and the `update$apply` one-call PROBE→APPLY→VERIFY→PERSIST→ROLLBACK pipeline (§5/§11) is the agent's preferred path. **Note:** update-agent is no longer debug-agent's promotion target — debug-agent dropped its `debug$promote-hot-patch` hand-off and now makes its own permanent fixes (see `debug-agent-design.md`). That change does not affect update-agent itself.
>
