...
3. **First-class parallel stages?** Pipelines support per-stage `:parallel?` markers; the executor can run independent stages concurrently. workflow-agent currently sequences moves one per iteration (CoAct's contract). Two paths: (a) the LLM emits ONE clojure fence that `par-map`s over the stage calls — works today via CoAct (`<!-- ParallelBlock -->` will not do it: the marker only parallelizes bash/python/js blocks); (b) extend the helpers to manage cross-stage dependency tracking automatically. Suggestion: start with (a); revisit (b) if benchmarks show meaningful latency wins.
