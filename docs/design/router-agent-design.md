...
> the former `router-agent-lightweight-redesign.md` has been folded in here and
> removed.
>
> **As-built (verify against `common/router_agent.clj`, `common/main.clj`,
> `common/router_agent_hooks.clj`):**
> - **router-agent is the front-door router.** Each user turn it routes to the
>   right specialist (or answers inline via CoAct's own answer / tool / code
>   channels). Routing is pure LLM judgment over the §6 decision table; router-agent
...
>   `:agent.session/closed` hook (`common/router_agent_hooks.clj`).
...
>   (`router$append-log`, now **retired** as a tool) onto a **turn-derived hook**
...
>   `routing.log` format is unchanged, so `router$resume?`/`router$last-shape` parse it
>   identically. See §4.2, §5, §10.
> - **Five hooks shipped** in `common/router_agent_hooks.clj` (not the three this
...
>
> **Scope:** `components/agent/src/ai/brainyard/agent/common/router_agent.clj`,
> `common/main.clj`, `common/router_agent_hooks.clj`
...
1. **Routing is the agent's identity.** router-agent does not solve sub-problems itself. Its job is to pick the right specialist (or coact channel) per question shape, hand off cleanly, and report back. When it does answer directly (greetings, factual recall, single-fact lookups), it uses CoAct's own answer channel — same substrate, no separate codepath.

2. **Specialists own their domains; router-agent owns the seam between them.** router-agent never writes to `.brainyard/agents/plan-agent/`, `.brainyard/agents/todo-agent/`, `.brainyard/agents/exec-agent/`, etc. Those are specialist territories. router-agent writes only to `.brainyard/agents/router-agent/<session-id>/routing.log` and reads sibling outputs through their cherry-picked read-helpers.
...
3. **The instruction encodes the routing decisions; routing is judgment.** A decision-table layer in the instruction (§6) names every question shape and the right move. The LLM uses the table; the BT does not branch on it. Same `coact-loop-subtree`. The deciding — pick the move, state a one-sentence reason — is inherent LLM capability and is left entirely to the model.
   ...
   3a. **The routing log is observation, not authoring.** Most of a routing-log line is *observable from the turn itself* — which specialist was dispatched, what `Saved <kind>:` artifact came back, the user's question, the turn/iter counters. So a hook derives the line; the model never assembles a structured map. This is the router-agent analog of eval's "scoring is judgment" and research's "orchestration is judgment": the *deciding* is inherent; only the *logging* was over-tooled. See §5.

4. **Small tool registry.** router-agent reaches the world through (a) every other `defagent` via direct kebab-case dispatch (they are in `!tool-defs`), (b) basic CoAct primitives for the routing log (read-file / write-file / bash), (c) bootstrap discovery (list-tools / get-tool-info / search) so the agent can introspect when an unfamiliar question requires it. That's it.

5. **Acceptance lives one level down.** router-agent does NOT manage acceptance criteria itself. If a question is research-shaped, acceptance lives in the research dossier; if workflow-shaped, in the workflow dossier; if it's a single specialist call, in that specialist's pre/post-flight. The front-door agent has nothing to commit to — its only contract is "I picked the right downstream agent and surfaced their result coherently."

6. **No clone-self recursion.** `query$clone` is excluded — calling router-agent from inside router-agent is the depth-2 anti-pattern. Calling *other* agents through their kebab-case dispatch is flat call-tool routing and IS the design.

7. **Generous iteration cap, but typically uses few.** Default 20 iterations (CoAct's default). Most user questions resolve in 1–3 iterations (route → one specialist call → finalize). The budget exists for questions that need light pre-research before routing, or for multi-specialist questions where router-agent threads two specialists itself rather than handing the whole arc to research-agent.

8. **Session-scoped routing memory.** The routing log is per agent-session (the shared `:user-id`/`:session-id` map), not per-question. Subsequent questions in the same session read the prior log to decide whether a routing decision is already in progress or settled. A research-agent run that the user just initiated stays visible to subsequent router-agent turns in the same session.

9. **Honest hand-off.** When router-agent delegates to a specialist, the specialist's `Saved <kind>: <path>` contract line is surfaced verbatim in router-agent's `:answer` so the user always sees the durable artifact path. The user can then `bb tui run -a <specialist> "@<id>"` to resume directly with the specialist on a future turn — router-agent's routing was a one-time decision, not a sticky binding.

10. **Falls back to coact-agent's channels.** If no specialist fits, router-agent uses CoAct's own tool / code / answer channels directly — same registry, same sandbox, same finalize. router-agent is a *thin specialization* of coact-agent, not a replacement.
...
coact-agent  (substrate — full BT, sandbox, router, accumulator)
  ...
  └─ router-agent        (front-door router — the user-facing entry)
```

`router-agent` does **not** replace any existing agent. It is the layer above all of them.

| Question shape | Use | Why |
|---|---|---|
| "Hi" / "What is 2+2?" / "Tell me about CoAct" | router-agent answers directly (answer channel) | Conversational / direct knowledge; no tool needed. |
| "What's in this file?" / "List tools matching X" | router-agent uses tool channel (read-file / list-tools / get-tool-info) | One-shot RPC; no specialist warranted. |
| "Find me where the loop guard lives" | router-agent → explore-agent | Discovery across files/web/MCP/skills. |
| "Rename foo to bar in src/x.clj" | router-agent → edit-agent | Safe single-file edit; no plan/todo arc. |
| "Compose three tool results into a table" | router-agent uses code channel (clojure fence) | Composition; no specialist. |
| "Search Slack for messages about Q4 launches" | router-agent → mcp-agent (or tool channel if direct call sufficient) | MCP read/write; specialist owns lifecycle. |
| "Draft a plan to migrate the auth middleware" | router-agent → plan-agent | Plan authoring; pre/post-flight gating. |
| "Spawn a todo from plan migration-auth" | router-agent → todo-agent | Decomposition from existing plan dossier. |
| "Drive the migration-auth todo to completion" | router-agent → exec-agent | Per-item routing; delegates writes. |
| "Score whether migration-auth met its acceptance" | router-agent → eval-agent | Verdict against plan acceptance. |
| "Research how to reduce `bb tui` cold start to <1s and implement it" | router-agent → research-agent | Multi-specialist research thread with dossier. |
| "Run the feature-launch workflow for feature F" | router-agent → workflow-agent | Multi-stage domain workflow. |
| "Summarize patterns across 200 log files" | router-agent → rlm-agent | MapReduce over too-big context. |
| "Save this note to memory" / "What do you remember about X?" | router-agent → memory-agent | Long-term memory ops. |
| "Bootstrap BRAINYARD.md for this repo" | router-agent → init-agent | Project bootstrap. |
| "Help me prepare for a meeting using ACP" | router-agent → acp-agent | External agent backend. |

Rule: **router-agent is for the first hop**. After hand-off, the specialist owns the arc. On the next user turn, router-agent looks at the routing log to decide whether to resume the same specialist (`bb tui ask "@<id>"`) or route fresh.

---

## 4. The Routing Log — `.brainyard/agents/router-agent/<session-id>/`

The single piece of state router-agent itself owns. Thin, append-only, session-scoped.
...
The `<session-id>` is the agent-session's `:session-id` (the same identity carried by sub-agents). router-agent does NOT manufacture its own ids — it piggybacks on the session machinery already in place (see `docs/core/agent.md` §Two-layer session model).

### 4.2 `routing.log` — NDJSON Decision Trail (hook-written)

One line per routing decision. Cheap to read selectively (jq, grep, or `read-file :lines [N M]`). The line is **written by the `:agent.ask/post` routing-line hook**, not by the LLM — router-agent no longer calls `router$append-log` (retired as a tool). The hook derives `routed-to`/`shape`/`artifact` from the turn and lifts `reason` from the model's answer; the format below is unchanged so `router$resume?`/`router$last-shape` parse it identically. See §5 for the derivation.
...
| `iter` | int | router-agent iteration within that turn. |
...
Both files are append-only. `pointers.md` is auto-captured by the `:agent.tool-use/post` hook (§10.2); on session close (`:agent.session/closed`), the `INDEX.md` at `.brainyard/agents/router-agent/` gets a one-line entry with the session-id, turn count, and the headline routing shapes seen.

### 4.4 The Routing Log as Hook-Derived Observation (the router-agent-specific move)

router-agent is the **least brittle** agent in the set — most of its bookkeeping was already hooks + read seams. Before the redesign there was exactly one authoring micro-tool left: `router$append-log`, which made the model assemble a per-move map `{:turn :iter :question :shape :routed-to :artifact :reason}` and classify `:shape` against the multi-element enum every single turn. The instruction leaned on it hard (*"the log is the contract with the next turn; failing to update it makes resume-shaped questions ambiguous"*) — the same per-turn-obligation pressure with the same failure mode: a smaller model skips it, and continuation detection silently degrades. Because the root agent runs on *every* turn, that tax was paid the most often here.
...
(def main-tools
  (vec (distinct
         (concat
           ;; Filesystem — for router-agent's OWN routing-log dir.
           ...
           ;; Discovery — router-agent answers some questions directly via tool
           ...
           ;; Lets router-agent inspect upstream artifacts cheaply when deciding
           ...
           ;; router$* helpers (see §8) — session-id / resume? / bootstrap /
           ;; append-pointer / last-shape / index-append. router$append-log is
...
The instruction's job (§6) is to teach the LLM **when to reach for each specialist**. The roster above is the *substrate* router-agent uses for everything else (routing log, direct answers, light pre-research before routing).

What is *deliberately omitted*:

| Excluded | Reason |
|---|---|
| `query$clone` | Clones router-agent itself = clone-self recursion. Forbidden. Cross-agent dispatch is direct kebab-case + flat. |
| `router$append-log` (as an LLM tool) | **Retired.** The per-turn routing line is hook-derived (§5, §10) — the model no longer assembles it. A private `append-log!` fn renders the same NDJSON line and is called only from the routing-line hook. |
...
| `mcp$tools :call` (write-side), `skills$write` / `skills$install` | These belong to mcp-agent / skill-agent. router-agent routes; it does not bypass. |
...
> The full instruction body lives in `components/agent/src/ai/brainyard/agent/common/router_agent.clj` under `(def ^:private instruction …)`. This section reproduces the section headers + key contracts; consult the source for the authoritative current text.
...
log helper (router$append-log is retired). Your only obligations:
...
HARD RULES
────────────────────────────────────────────────────────────────────────────
1. STAY FLAT — no clone-self dispatch. `query$clone` clones router-agent itself
...
|---|---|---|
| `router$session-id` | `(router$session-id)` → `{:session-id "<id>"}` | Returns current agent-session id. |
| `router$resume?` | `(router$resume? :session-id …)` → `{:exists? bool :turn-count int :last-shape kw :last-artifact "<path>"}` | Cheap probe — does the routing-log dir exist + its last state? |
| `router$bootstrap` | `(router$bootstrap :session-id …)` → `{:dir … :log-path …}` | Creates dir + initial routing.log + pointers.md. Idempotent. Normally unnecessary — the `:agent.session/created` hook bootstraps for you. |
| `router$append-pointer` | `(router$append-pointer :session-id … :path "<path>" :caption "<one-line>")` → `{:appended true}` | One bullet into pointers.md. Usually the `:agent.tool-use/post` hook captures specialist `Saved <kind>:` lines for you. |
| `router$last-shape` | `(router$last-shape :session-id …)` → `{:shape :plan-author :routed-to … :artifact … :turn N}` | Last routing decision in the current session — drives continuation detection. |
| `router$index-append` | `(router$index-append :session-id … :turn-count N :shapes [...])` → `{:appended true}` | Appended on session close by the `:agent.session/closed` hook; you rarely call it. |
...
## 8. `(router$*)` Sandbox Helpers

Mirrors the helpers introduced in `research-agent`, `workflow-agent`, and `explore-agent`. They live in `ai.brainyard.agent.common.router`, register as `defcommand`s, and surface in the sandbox via auto-binding. They are **read seams + accessors** — the only structured-authoring helper, `router$append-log`, was **retired as an LLM tool** in the redesign: a private `append-log!` fn renders the same NDJSON line and is called only from the routing-line hook (§5, §10.4). The bound roster (`router-helpers`) is `router$session-id`, `router$resume?`, `router$bootstrap`, `router$append-pointer`, `router$last-shape`, `router$index-append`.

| Helper | Signature | What it does |
|---|---|---|
| `router$session-id` | `(router$session-id)` → `{:session-id "<id>"}` | Returns the current agent-session id (reads `*current-agent*` and pulls the `:session-id` via `proto/session-id`). A runtime accessor — the model cannot construct it. |
| `router$resume?` | `(router$resume? :session-id "<id>")` → `{:exists? bool :line-count int :turn-count int :last-shape kw :last-artifact "<path>"}` | Cheap probe: does the routing-log dir exist? How many lines/turns has it seen? What was the last decision shape/artifact? Single pass over `routing.log`. |
| `router$bootstrap` | `(router$bootstrap :session-id "<id>")` → `{:dir "<path>" :log-path "<path>" :pointers-path "<path>" :exists? bool}` | Creates `.brainyard/agents/router-agent/<session-id>/` with empty `routing.log` and a header in `pointers.md`. Idempotent — if dir exists, returns its current state instead of overwriting. **Normally driven by the `:agent.session/created` hook**, so the model rarely calls it. |
| `router$append-pointer` | `(router$append-pointer :session-id … :path "<path>" :caption "<one-line>")` → `{:appended true}` | Appends one markdown bullet to `pointers.md` with a timestamp + caption. Usually unnecessary — the `:agent.tool-use/post` hook auto-captures specialist `Saved <kind>:` lines. |
| `router$last-shape` | `(router$last-shape :session-id …)` → `{:exists? bool :shape kw :routed-to "<agent>" :artifact "<path>" :turn N :iter M :question "…" :reason "…"}` | Reads + parses the last line of `routing.log`. Drives continuation detection ("user said 'now do X with the thing' — what was the thing?"). |
| `router$index-append` | `(router$index-append :session-id … :turn-count N :shapes [:plan-author :decompose :execute])` → `{:appended true}` | Append a one-line entry to `.brainyard/agents/router-agent/INDEX.md`. Driven by the `:agent.session/closed` hook. |
...
`router-agent` does **not** define its own BT. `run-coact-derived` falls back to `coact-agent`'s `:bt-factory`:
...
The `coact-loop-subtree`'s router (answer / code / tool / repair) handles all four channels router-agent uses; the decision table in the instruction (§6) tells the LLM *which* channel to pick per question shape and *which specialist to dispatch* when the tool channel is the answer.

Default iteration cap stays at **20** — router-agent is a router, not a worker. Most turns resolve in 1–3 iterations. The cap can be raised via `agent-runtime$config :key "max-iterations" :value "N"` for sessions where the user wants router-agent to drive light pre-research before delegating.

No new BT actions, no new schemas, no SCI binding additions are required for the baseline (beyond the optional `router$*` helpers in §8).

---

## 10. Hooks Integration

router-agent uses the existing hooks registry (see `docs/core/agent.md` §Hooks) — no new event types. They ship in `common/router_agent_hooks.clj`, all `:source ::router-agent`, self-installing at namespace load via `(install!)` and each wrapped in try/catch so a hook failure never propagates into the user answer. **Five** handlers ship (the redesign added the `:agent.ask/pre` snapshot + the `:agent.ask/post` routing-line recorder — together they make the routing log hook-derived, retiring `router$append-log`):
...
A side-effecting handler (`routing-log-bootstrap`) runs on session creation to ensure `.brainyard/agents/router-agent/<session-id>/` exists before the first user turn. Idempotent, and runs for every session (any agent may invoke router-agent later).

```clojure
(hooks/register-hook!
  :agent.session/created
  ::routing-log-bootstrap
  routing-log-bootstrap            ; (fn [{:keys [session-id]}] (router/router$bootstrap :session-id session-id))
  :source ::router-agent)
```

### 10.2 `:agent.tool-use/post` — capture `Saved <kind>:` lines

A read-only observer parses every sub-agent's result for `Saved <kind>: <path>` lines and writes the corresponding `pointers.md` bullet. This means router-agent's instruction doesn't have to inline the parsing logic — the hook does it as a side effect.

```clojure
(hooks/register-hook!
  :agent.tool-use/post
  ::capture-saved-artifacts
  capture-saved-artifacts          ; router-agent? + (specialist-agents tool-name) →
                                   ; (doseq [{:keys [kind path]} (router/parse-saved-lines answer)]
                                   ;   (router/router$append-pointer :session-id sid :path path :caption …))
  :source ::router-agent
  :match  (fn [{:keys [agent]}] (router-agent? agent)))
```

This decouples artifact tracking from instruction discipline — even if the LLM forgets to call `router$append-pointer` itself, the hook catches it. `pointers.md` was already hook-driven before the redesign and stays so.
...
The **sole writer** of `routing.log`. router-agent no longer calls `router$append-log`; this hook derives the per-turn line from the turn itself (§5) and appends it via the private `router/append-log!`:
...
Because the hook observes rather than asks the model to author, the trail can't be forgotten — strictly better than the old "model skipped `router$append-log` entirely" failure mode. Shape validation moved from an eager hard-reject to a parse-time coerce (`:unspecified` fallback), so a mis-derived shape never fails the turn.

### 10.5 `:agent.session/closed` — finalize INDEX.md

Appends one line to `.brainyard/agents/router-agent/INDEX.md` summarizing the session's turn count and distinct routing shapes (no-op when router-agent was never invoked).

```clojure
(hooks/register-hook!
  :agent.session/closed
  ::finalize-index
  finalize-index                   ; reads router/read-routing-log → router/router$index-append
  :source ::router-agent)
```
...
````markdown
```clojure
(def sid (:session-id (router$session-id)))
(router$resume? :session-id sid)
...
````markdown
```clojure
;; router$last-shape sees :plan-author + plan path. "now spawn a todo from it" is
;; a clear continuation → G. DECOMPOSE → todo-agent.
(def last (router$last-shape :session-id sid))
...
todo-agent returns with `Saved todo: …` + `Saved dossier: …`. router-agent surfaces the lines and the post.acceptance_coverage summary in the final `:answer`.
...
````markdown
```clojure
(def log-lines (read-file {:path ".brainyard/agents/router-agent/sess-7f3a/routing.log"}))
...
Note: turn 3 uses NO specialist — router-agent answers from the routing log directly. This is `S. META-RESUME` from §6's decision table. Because there's no dispatch for the hook to infer from, the model adds the `Routing: meta-resume — …` line so the `:agent.ask/post` hook records the shape (`routed-to: null`).
...
| **Routing log corruption** | NDJSON parse fails on resume. | `router$resume?` validates the log; on parse failure, falls through to `:exists? false :recovery-needed true`. The instruction handles this as a CLARIFY (T): surface the broken state and offer to bootstrap fresh. |
| **Cycle: user keeps asking variants of the same question** | Same shape routes to the same specialist repeatedly with no progress. | The loop-guard hook (`common/loop_guard_hook.clj`) is already registered as a `:agent.tool-use/pre` gate and catches pathological repeat-call patterns. router-agent inherits it for free. |
| **Specialist hits its own iteration cap mid-arc** | User sees `Hold:` or `abandoned` status without router-agent's framing. | Specialist's `Saved dossier:` line is surfaced verbatim; router-agent's `:answer` adds a one-paragraph "what this means + recommended next move" layer on top. |
| **Concurrent router-agent instances in the same session** | Routing log race. | `routing.log` writes use `:append true` (atomic on POSIX for small writes); the per-session directory has a lock (mirroring `agent-tui-persist`'s session-store lock). |
...
router-agent is a thin specialization of coact-agent. The diffs:

| | coact-agent | router-agent |
|---|---|---|
...
| **Tool roster** | Generic CoAct tools (file/shell/web/bootstrap/invocation/discovery). | coact-agent's roster **plus** the read-only sibling dossier helpers + the `router$*` helpers namespace (read seams/accessors — no `router$append-log`). |
| **State** | None of its own; sandbox `def`s reset per agent instance. | Persists `.brainyard/agents/router-agent/<session-id>/routing.log` + `pointers.md` — both **hook-written**, not LLM-authored. |
| **Default `:max-iterations`** | 20. | 20 (most turns finish in 1–3). |
| **Hooks** | Inherits the global registry. | Adds **five** handlers (`:agent.session/created`, `:agent.ask/pre`, `:agent.tool-use/post`, `:agent.ask/post`, `:agent.session/closed`) under `:source ::router-agent` — the `:agent.ask/post` recorder is the sole writer of `routing.log`. |
...
The minimal diff is intentional. Anything coact-agent already does well (channel discipline, sandbox state, parallel blocks, finalize) router-agent inherits unchanged. The only new thing is the *routing knowledge*.

---

## 14. Migration — Complete

router-agent shipped as additive, alongside coact-agent — no code was removed.

### 14.1 The lightweight, hook-derived redesign (2026-06, done)

The redesign removed router-agent's last per-turn structured-authoring obligation. It landed as:
...
2. **`router$append-log` retired** as an LLM tool. Its NDJSON rendering moved to the private `router/append-log!`, called only from the hook. The bound roster (`router-helpers`) dropped it.
...
4. **`routing.log` format unchanged** (same NDJSON keys) — only the *writer* moved from the model to the hook, so `router$resume?` / `router$last-shape` parse it identically. **No data migration.**

The change paired naturally with the base-substrate rollout (router-agent is the chief beneficiary of the inline track/execute/edit substrates — see [the synthesis](./agent-lightweight-redesign-synthesis.md)), but was independent and could land any time.

### 14.2 Front-door entry & compatibility

- **Sub-agent dispatch unaffected.** coact-agent is a leaf-level reasoning style; nothing dispatches to it as a substrate. router-agent is the *user-facing* front door, not a substrate. coact-agent stays available via explicit `-a coact-agent`.
- **Compatibility with `bb tui ask`.** `bb tui ask -m opus 'What is 2+2?'` works — router-agent's `A. DIRECT-ANSWER` move handles it in one iteration via the answer channel; the routing-log overhead is one hook-written NDJSON line.
- **ACP & web app.** `bases/agent-web` and `bases/acp-stub-agent` choose their own entry agent; a backend wanting the front-door behavior switches its entry to `router-agent`. Default per-base resolution lives in each base's `core.clj`.
...
| File | Purpose |
|---|---|
| `components/agent/src/ai/brainyard/agent/common/router_agent.clj` | The `defagent` form + instruction + tool-context + roster wiring. Routing-log obligation is hook-recorded; the instruction states only the reason + the `Routing:` self-answer convention. |
| `components/agent/src/ai/brainyard/agent/common/main.clj` | The `router$*` helpers (`router$session-id`, `router$resume?`, `router$bootstrap`, `router$append-pointer`, `router$last-shape`, `router$index-append`) as `defcommand`s, plus the **private** `append-log!` + `coerce-shape` (the 22-keyword `valid-shapes`) and `parse-saved-lines`. `router$append-log` is **not** a bound tool. |
| `components/agent/src/ai/brainyard/agent/common/router_agent_hooks.clj` | The **five** hook handlers (bootstrap / capture-pre-turn / capture-saved-artifacts / record-routing-line / finalize-index), self-installing at load. |
| `components/agent/test/ai/brainyard/agent/router_agent_test.clj` | Tests: routing-log bootstrap idempotency, decision-table dispatch, session continuation detection, hook-derived routing line + self-answer `Routing:` parsing + `:unspecified` fallback. |
| `docs/design/router-agent-design.md` | This document (as-built). |
| `docs/core/agent.md` | Lists `router-agent` in the built-in agents table. |

---

## 16. Open Questions

1. **Should router-agent surface a routing-decision "reason" to the user, or keep it internal to routing.log?** **Resolved (as-built):** the model states a one-sentence routing reason at the top of `:answer` ("Routed to plan-agent — your question reduced to plan authoring."), which the `:agent.ask/post` hook lifts into `routing.log` — so the reason is both user-visible and logged, with no separate authoring step. A `:show-routing? true|false` runtime knob to mute the user-facing line remains a possible follow-up.
...
3. **How does router-agent interact with `acp-agent` when ACP is the front door?** Today an ACP client sends prompts directly to a brainyard agent. If the ACP backend wires its entry to `router-agent`, the routing-log lives on the brainyard side and the ACP client gets back final `:answer` strings + Saved-line paths. Multi-hop ACP (ACP → router-agent → ACP-bridged third-party agent) is out of scope for v1.

4. **Should router-agent's decision table be data-driven (EDN) instead of prompt-embedded?** Current proposal: embedded in the instruction (mirrors research-agent's approach). A future revision could lift `:shape :routed-to :examples` into `.brainyard/agents/router-agent/decision-table.edn` so power users can tune routing without forking the agent file. This is a "phase 3" idea.

5. **Should the read-only cherry-picks include `workflow$read-dossier` and `research$read-dossier` when those agents emit dossiers router-agent might inspect?** Probably yes — to be added once those agents stabilize their dossier formats (research-agent has; workflow-agent is still design-phase).

6. **What's the right default model for router-agent?** Routing is cheap (typically <500 tokens of LLM output per turn). A small/fast model (haiku) is plausibly fine and saves cost. But misroutes are expensive. Current proposal: same default as coact-agent (`claude-code:haiku`), with a runtime knob to bump to sonnet for high-stakes sessions.

---

## 17. Summary

`router-agent` is the front-door router for brainyard: a thin CoAct specialization whose entire job is to look at a user question and pick the right specialist (or coact-agent's own channels) per question shape. The substrate, the loop, the sandbox — all inherited from coact-agent. What it adds:
...
- A **self-recording routing log** under `.brainyard/agents/router-agent/<session-id>/` that gives session continuity, audit trail, and continuation detection — **hook-written, not LLM-authored**. The model routes and states a reason; the `:agent.ask/post` hook derives and appends the structured line (`router$append-log` retired).
