...
| `config$revert` | `{:snapshot-path <path>}` or `{:steps <int>}` | Restores a prior snapshot; itself snapshots the current file first (so revert is reversible). **As-built:** also drops the cached `!global-config` (`:reloaded? true`), or the revert would restore the file and leave the running process on the values it was reverting away from. |
| `config$reload` | `{}` | Re-reads `config.edn` into the running process and reports what actually changed. Pure read. The persisted layer is loaded lazily ONCE per process and invalidated only by `config$apply`/`config$revert`, so a file changed by any other route — a hand edit, the wizard, `git pull`, a sibling `by` — never reaches a running session. Separates `:changed` (effective, in force), `:shadowed` (the file moved but `:env`/`:agent`/`:session` still wins, with the winning `:layer` and the `:effective` value) and `:requires-restart` (changed, but read once at boot). Also returns the two file-shape defects `load-global-config!` otherwise only logs: `:unknown-keys` and `:misplaced-keys`. |
