...
(ns ai.brainyard.agent.router-agent-test
  "Tests for router-agent (lightweight redesign): registration, inherited
   bt-factory (CoAct), the curated agent-tools roster (the per-turn routing
   line is now HOOK-DERIVED, so router$append-log is retired as an LLM tool),
   instruction-content anchors, unit tests for the surviving router$* seams
  ...
  (:require [ai.brainyard.agent.common.router :as router]
            ...
            [ai.brainyard.agent.common.router-agent-hooks :as router-hooks]
...
(defn- tempdir
  ...
  []
  (-> (Files/createTempDirectory "router-agent-test-" (make-array FileAttribute 0))
      .toFile
      .getAbsolutePath))
...
(deftest registration-test
  (testing "router-agent is registered as an agent"
    (let [agent-defs (tool/get-tool-defs :type :agent)]
      (is (contains? agent-defs :router-agent))
      (let [agent-def (get agent-defs :router-agent)]
        (is (= :router-agent (:id agent-def)))
...
(deftest inheritance-test
  ...
  (let [m-def (get (tool/get-tool-defs :type :agent) :router-agent)]

    (testing "router-agent's :fn is the wrap-fn (CoAct entry)"
      (is (some? (:fn m-def))))

    (testing "router-agent pins :bt-factory explicitly (so setup-agent-by-id picks it up)"
    ...
    (testing "default :max-iterations is 20 (CoAct default — router-agent is a router, not a worker)"
      (let [bt-factory (get-in m-def [:meta :bt-factory])
            bt (bt-factory {})]
        (is (some? bt))))))
...
(deftest agent-tools-roster-test
  (let [m-def (get (tool/get-tool-defs :type :agent) :router-agent)
    ...
    (testing "the surviving router$* seams are present; router$append-log is retired"
      (is (contains? ids :router$session-id))
      (is (contains? ids :router$resume?))
      (is (contains? ids :router$bootstrap))
      (is (contains? ids :router$append-pointer))
      (is (contains? ids :router$last-shape))
      (is (contains? ids :router$index-append))
      (is (not (contains? ids :router$append-log))
          "the per-turn routing line is hook-derived — router$append-log is not an LLM tool"))
...
(deftest instruction-content-anchors-test
  (let [m-def (get (tool/get-tool-defs :type :agent) :router-agent)
    ...
    (testing "session-probe + hook-recorded routing line are documented"
      (is (str/includes? ins "router$resume?"))
      ;; The routing line is hook-derived now — the instruction must NOT tell the
      ;; LLM to call router$append-log, and must teach the self-answer convention.
      (is (not (str/includes? ins "router$append-log")))
      (is (str/includes? ins "Routing: <shape>"))
      (is (str/includes? ins "hook")))))

;; ============================================================================
;; Unit tests for router$* helpers
;; ============================================================================
...
(deftest bootstrap-idempotence-test
  (let [base (tempdir)
    ...
    (testing "first call creates dir + empty routing.log + pointers.md header"
      (let [r (router/router$bootstrap :session-id sid :base-dir base)]
        (is (some? (:dir r)))
        (is (some? (:log-path r)))
        (is (some? (:pointers-path r)))
        (is (not (:exists? r)))
        (is (.isFile (io/file base ".brainyard/agents/router-agent" sid "routing.log")))
        (is (.isFile (io/file base ".brainyard/agents/router-agent" sid "pointers.md")))
        (let [pointers-body (slurp (io/file base ".brainyard/agents/router-agent" sid "pointers.md"))]
          (is (str/includes? pointers-body "# Session pointers")))))

    (testing "second call is idempotent — returns :exists? true, does not overwrite"
      (spit (io/file base ".brainyard/agents/router-agent" sid "routing.log")
            "{\"turn\":1,\"shape\":\"direct-answer\"}\n")
      (let [r (router/router$bootstrap :session-id sid :base-dir base)]
        (is (:exists? r))
        (is (some? (:dir r))))
      ;; The hand-spat line should still be there.
      (let [log-body (slurp (io/file base ".brainyard/agents/router-agent" sid "routing.log"))]
        (is (str/includes? log-body "direct-answer"))))))

(deftest coerce-shape-test
  (testing "known shapes pass through (keyword or string)"
    (is (= :plan-author (router/coerce-shape :plan-author)))
    (is (= :explore (router/coerce-shape "explore")))
    (is (= :direct-answer (router/coerce-shape :direct-answer))))
  (testing "unknown / nil → :unspecified (never fails the turn)"
    (is (= :unspecified (router/coerce-shape :not-a-real-shape)))
    (is (= :unspecified (router/coerce-shape "bogus")))
    (is (= :unspecified (router/coerce-shape nil)))))

(deftest append-log!-test
  ;; append-log! is the internal NDJSON writer the routing-log hook calls
  ;; (router$append-log was retired as an LLM tool). It coerces unknown shapes
  ...
  (let [base (tempdir)
    ...
    (testing "creates the routing-log dir if missing (no router$bootstrap needed)"
      (let [r (router/append-log!
        ...
        (is (:appended r))
        (is (.isFile (io/file base ".brainyard/agents/router-agent" "never-bootstrapped" "routing.log")))))))
...
(deftest last-shape-roundtrip-test
  (let [base (tempdir)
    ...
    (testing "returns :exists? false when log is empty"
      (let [r (router/router$last-shape :session-id sid :base-dir base)]
        (is (not (:exists? r)))))
...
(deftest append-pointer-test
  (let [base (tempdir)
    ...
    (testing "appends a markdown bullet with the path and caption"
      (let [r (router/router$append-pointer
               :session-id sid :base-dir base
               :path ".brainyard/agents/plan-agent/plans/migrate-auth.md"
               :caption "plan body — migrate-auth")]
        (is (:appended r)))
      (let [body (slurp (io/file base ".brainyard/agents/router-agent" sid "pointers.md"))]
...
(deftest resume-probe-test
  (let [base (tempdir)
    ...
    (testing ":exists? false before bootstrap"
      (let [r (router/router$resume? :session-id sid :base-dir base)]
        (is (not (:exists? r)))))
...
(deftest index-append-test
  (let [base (tempdir)]
    (testing "first call creates INDEX.md and writes one line"
      (let [r (router/router$index-append
               :session-id "s-1" :base-dir base
               :turn-count 3 :shapes [:plan-author :decompose :execute])]
        (is (:appended r))
        (is (some? (:line r))))
      (let [body (slurp (io/file base ".brainyard/agents/router-agent/INDEX.md"))]
...
(deftest parse-saved-lines-test
  (testing "extracts every Saved <kind>: <path> line"
    (let [text "plan-agent drafted a 6-item approach.\n\nSaved plan: .brainyard/agents/plan-agent/plans/migrate-auth.md\nSaved dossier: .brainyard/agents/plan-agent/dossiers/20260516-091412-migrate-auth.md\n\nNext: spawn a todo."
          out  (router/parse-saved-lines text)]
      (is (= 2 (count out)))
  ...
  (testing "empty / blank text returns empty vector"
    (is (= [] (router/parse-saved-lines nil)))
    (is (= [] (router/parse-saved-lines "")))
    (is (= [] (router/parse-saved-lines "no saved lines here"))))

  (testing "rejects lines that don't start with `Saved <kind>:`"
    (is (= [] (router/parse-saved-lines "  Saved foo: indented but with space"))
...
(defn- mk-stub
  "Stub router-agent. `bt-mem` (optional) is an atom of short-term memory
...
(deftest capture-hook-test
  (let [base       (tempdir)
        ...
        bootstrap! #(router/router$bootstrap :session-id sid :base-dir base)]
    (bootstrap!)

    (testing "router-agent + specialist tool-call → captures Saved lines"
      ;; The hook's `router/router$append-pointer` call resolves base-dir from
      ...
      (with-redefs [config/project-dir (constantly base)]
        (router-hooks/capture-saved-artifacts
    ...
    (testing "non-router-agent caller → no capture"
      (let [pre-body (slurp (io/file base ".brainyard/agents/router-agent" sid "pointers.md"))]
        (with-redefs [config/project-dir (constantly base)]
          (router-hooks/capture-saved-artifacts
           {:agent not-main :tool-name "plan-agent" :result {:answer answer}}))
        (is (= pre-body (slurp (io/file base ".brainyard/agents/router-agent" sid "pointers.md"))))))
...
;;
;; The routing line is HOOK-DERIVED (router-agent no longer calls router$append-log):
...
(deftest record-routing-line-test
  (let [base (tempdir)
    ...
    (testing "specialist dispatch → routed-to + specialist→shape + artifact + reason"
      (let [stub (mk-stub :router-agent sid (bt-mem-with-dispatch "plan-agent"))]
        (with-redefs [config/project-dir (constantly base)]
          (router/router$bootstrap :session-id sid :base-dir base)
        ...
        (let [ln (first (router/read-routing-log sid :base-dir base))]
          (is (= 1 (count (router/read-routing-log sid :base-dir base))))
    ...
    (testing "self-answered move → shape + reason parsed from the `Routing:` line"
      (let [stub (mk-stub :router-agent sid)]  ; no dispatch
        (with-redefs [config/project-dir (constantly base)]
          (router-hooks/capture-pre-turn {:agent stub})
          (router-hooks/record-routing-line
           {:agent stub
    ...
    (testing "unknown `Routing:` shape → coerced to :unspecified (never fails)"
      (let [stub (mk-stub :router-agent sid)]
        (with-redefs [config/project-dir (constantly base)]
          (router-hooks/capture-pre-turn {:agent stub})
          (router-hooks/record-routing-line
           {:agent stub
    ...
    (testing "idempotent — a re-fire on the same snapshot does not double-write"
      (let [stub (mk-stub :router-agent sid)
            pre  (count (router/read-routing-log sid :base-dir base))]
        (with-redefs [config/project-dir (constantly base)]
          (router-hooks/capture-pre-turn {:agent stub})  ; snapshots current max
          (router-hooks/record-routing-line
          ...
          (router-hooks/record-routing-line
           {:agent stub :input {:question "x"} :result {:answer "Routing: clarify — ambiguous"}}))
        (is (= (inc pre) (count (router/read-routing-log sid :base-dir base)))
    ...
    (testing "Non-router-agent caller → no line"
      (let [pre  (count (router/read-routing-log sid :base-dir base))
