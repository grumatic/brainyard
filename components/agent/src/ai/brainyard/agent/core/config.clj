  ...
  {:max-output-tokens       {:type "integer" :default 0
   ...
   :grapheme-width             {:type "keyword"
                                :env-fn #(if-let [v (not-empty (System/getenv "BY_GRAPHEME_WIDTH"))]
                                           (keyword v) ::env-unset)
                                :default :auto
                                :doc "How the TUI measures the width of emoji/CJK graphemes: :auto (default) negotiates DEC private mode 2027 once per terminal via DECRQM and caches the answer in <user-config-dir>/terminal-caps.edn | :off counts per codepoint, matching a terminal without 2027 | :on forces grapheme clustering with no probe. The regimes differ by up to 6 columns on one glyph (a ZWJ family emoji is 8 columns per-codepoint, 2 clustered), so the wrong one drifts every right-hand edge — and 2027 is default-ON in Windows Terminal, WezTerm, Ghostty, Contour and foot, which is why :auto is the default. Every failure mode (no tty, no reply, unparseable reply, exception) resolves to per-codepoint, so :auto can only ever be as wrong as :off. :auto skips the probe inside tmux, which never answers and uses wcwidth itself. Env: BY_GRAPHEME_WIDTH."}
