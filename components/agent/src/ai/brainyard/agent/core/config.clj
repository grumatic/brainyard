...
(def config-schema
  ...
  {:max-output-tokens       {:type "integer" :default 0
   ...
   :tool-approval-patterns     {:type "array"
                                :default []
                                :doc "Globs over registered tool names whose match requires human approval before the tool runs, enforced by the :agent.tool-use/pre gate in common/tool_permission.clj. Same glob shape as :script-bridge-tools and :mcp-allow-tools — * matches any run of chars and spans $, so \"user$tool$*\" covers user$tool$create and \"*\" is every tool. SHIPS EMPTY, so the gate is inert until configured. Approval honors [:permissions :mode]: :auto-approve runs without prompting, :deny-by-default refuses, :ask-each-time prompts and REFUSES when there is no interactive channel (fail-closed, like write-file). This is a blast-radius and legibility control, not a sandbox — an agent with a bash fence already runs arbitrary code. See docs/design/tool-permission-gate-design.md."}
   :tool-allow-tools           {:type "array"
                                :default []
                                :doc "Globs exempting a tool from :tool-approval-patterns, checked FIRST so a broad approval pattern can carry a narrow exemption (approve \"mcp$*\", allow \"mcp$linear$*\"). It is a LOCAL exemption: it stops THIS gate asking and cannot stop another gate refusing, because a hook allow is an abstention rather than a vote — so this does NOT bypass the MCP gate. :mcp-allow-tools remains the only key that does."}
