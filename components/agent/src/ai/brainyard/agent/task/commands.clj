...
(defcommand task$detail
  ...
  :output-schema [:map
                  ...
                  [:progress     {:optional true} [:map {:desc "Structured progress snapshot for a running subagent task — {:iteration :tools-completed :last-tool :last-tool-result :observation}. Read this instead of parsing the log tail to judge how far along a subagent is. Absent for non-subagent tasks and once terminal."}]]
