...
(def command-registry
  ...
  [["/activity"         " [show [dir [-p N]]|hide|toggle]" "Toggle activity side pane (Mode B)"
   ...
   ["/session"          " [N|subcmd] [args]"      "Manage TUI tabs + persisted sessions"
    {:completions [["list"   "List runtime tabs"]
                   ["new"    "Create new tab (optional agent-id)"]
                   ["close"  "Close tab N (default: current)"]
                   ["rename" "Rename current tab (also updates disk label)"]
...
(defn format-analytics-display
  "Format analytics results for TUI scroll region display.
   Input: analytics map with :pqs :waste :cost keys.
   Returns ANSI-styled string for scroll region output."
  [analytics]
  (when analytics
    (let [{:keys [pqs waste cost]} analytics
          cols   (terminal-columns)
          bar    (apply str (repeat (min cols 60) ansi/h-line))
          section (fn [title]
                    (str (ansi/style bar ansi/dim) "\n"
                         (ansi/header (str "  " title)) "\n"))]
      (str "\n"
           (ansi/style (apply str (repeat (min cols 60) ansi/h-line)) ansi/bright-cyan) "\n"
           (ansi/style "  Session Analytics" ansi/bold ansi/bright-cyan) "\n"
           (ansi/style (apply str (repeat (min cols 60) ansi/h-line)) ansi/bright-cyan) "\n"

           ;; PQS Section
           (section "Prompt Quality Score (PQS)")
           (let [score (or (:overall-score pqs) 0)
                 color (cond (>= score 80) ansi/bright-green
                             (>= score 50) ansi/bright-yellow
                             :else         ansi/bright-red)]
             (str "  Overall: " (ansi/style (str score "/100") ansi/bold color) "\n"))
           (when-let [dims (:dimensions pqs)]
             (str (ansi/muted (str "  Specificity:          " (format "%2d/25" (or (:specificity dims) 0)))) "\n"
                  (ansi/muted (str "  Task Atomicity:       " (format "%2d/25" (or (:task-atomicity dims) 0)))) "\n"
                  (ansi/muted (str "  Context Completeness: " (format "%2d/20" (or (:context-completeness dims) 0)))) "\n"
                  (ansi/muted (str "  Acceptance Criteria:  " (format "%2d/20" (or (:acceptance-criteria dims) 0)))) "\n"
                  (ansi/muted (str "  Clarity:              " (format "%2d/10" (or (:clarity dims) 0)))) "\n"))
           (when (seq (:recommendations pqs))
             (str (ansi/muted "  Recommendations:") "\n"
                  (->> (:recommendations pqs)
                       (map #(str "    " (ansi/style ansi/arrow ansi/bright-yellow) " " (ansi/muted %)))
                       (str/join "\n"))
                  "\n"))
           ...
           (if (seq (:patterns waste))
             (str (ansi/warning (str "  Waste: "
                                     (format-number (long (or (:total-waste-tokens waste) 0)))
                                     " tokens ($" (format "%.4f" (double (or (:total-waste-cost waste) 0)))
                                     ", " (format "%.1f%%" (double (or (:waste-percentage waste) 0))) ")"))
                  "\n"
                  (->> (:patterns waste)
                       (map (fn [p]
                              (let [sev  (str/upper-case (name (or (:severity p) :low)))
                                    color (case (or (:severity p) :low)
                                            :high ansi/bright-red
                                            :medium ansi/bright-yellow
                                            ansi/dim)]
                                (str "  " (ansi/style (str "[" sev "]") color)
                                     " " (name (:pattern-id p)) ": "
                                     (ansi/muted (or (:detail p) ""))))))
                       (str/join "\n"))
                  "\n")
             (str (ansi/success "  No waste patterns detected.") "\n"))

           ;; Cost Section
           (section "Cost Analysis")
           (let [actual (:actual cost)]
             (str "  Actual cost: "
                  (ansi/style (str "$" (format "%.4f" (double (or (:total-cost actual) 0))))
                              ansi/bold ansi/bright-white)
                  (ansi/muted (str " (" (format-number (long (or (:total-tokens actual) 0)))
                                   " tokens, " (or (:call-count actual) 0) " calls)"))
                  "\n"
                  (when (pos? (or (:savings-potential cost) 0))
                    (str "  Potential savings: "
                         (ansi/style (str "$" (format "%.4f" (double (:savings-potential cost))))
                                     ansi/bold ansi/bright-green)
                         "\n"))
                  (when-let [tp (:throughput cost)]
                    (when (pos? (or (:output-tokens-per-sec tp) 0))
                      (str (ansi/muted (str "  Throughput: "
                                            (:output-tokens-per-sec tp) " tok/s, avg "
                                            (:avg-latency-ms tp) "ms"))
                           "\n")))))))))
