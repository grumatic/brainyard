...
> **v0.6.0 is live** ([release notes](https://github.com/grumatic/brainyard/releases/tag/v0.6.0) · [changelog](CHANGELOG.md)). **Agents can now talk to agents in other processes, and be talked to by them**: A2A (Agent2Agent — the Linux Foundation standard, JSON-RPC 2.0 over HTTP with SSE) closes both directions. A remote peer's skills register as ordinary agents, so a remote agent is asked with *exactly the same commands* as a local one — inheriting the reach policy, call-depth guard and eviction unchanged; there is deliberately no `a2a$ask`. `by a2a serve` exposes local agents inbound, and because that executes prompts against your workspace it is off by default, token-required, loopback-bound, and gated behind an empty allow-list. Plus a **user-scope project registry** (`by projects`), a `debug-agent` that can debug the shipped binary itself with no git checkout, and a bash tool that no longer hangs on a credential prompt. The **feature registry** over the config schema now spans 48 features across 11 families, queryable through `/feature` and `feature$*` — declared dependencies mean a gate set `true` can still resolve `off`, and `feature$explain` tells you which layer supplied the value and what's unmet. Opus remains the default model (`claude-code:opus` out of the box). Platform coverage: **macOS arm64** native binary plus a portable **JDK 21+ uberjar**; Linux and macOS amd64 binaries via the uberjar.

Brainyard is an **agent runtime** that happens to wear a terminal UI — not a chat box in front of a provider API. The shipping binary is `by`: it runs interactive TUI sessions, answers one-shot questions, and drives **26 built-in agents** across 10 subcommands (`run`, `ask`, `agents`, `models`, `config`, `sessions`, `projects`, `memory`, `events`, `a2a`). Providers: `claude-code` (the default — drives the local Claude CLI, no API key), `anthropic`, `openai`, `bedrock`, `google`, `groq`, `ollama`, `apple-fm`, plus OpenAI-compatible endpoints (`openrouter`, `together`, `fireworks`, `deepseek`, `mistral`, `azure`).
...
Pin a specific version with `BY_VERSION=v0.6.0` before piping to bash. See [`docs/install.md`](docs/install.md) for manual install, checksum verification, and troubleshooting.
