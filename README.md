...
> **v0.5.2 is live** ([release notes](https://github.com/grumatic/brainyard/releases/tag/v0.5.2) · [changelog](CHANGELOG.md)). **A feature registry over the config schema**: 43 features across 10 families, queryable through `/feature` and `feature$*` — declared dependencies mean a gate set `true` can still resolve `off`, and `feature$explain` tells you which layer supplied the value and what's unmet. Steer capability in bulk with family switches, `BY_FEATURES`, or profiles (`BY_PROFILE=minimal` turns off everything that spends an LLM call). **User-defined state machines** (`fsm$*`): a stateful reactor over the event bus, with durable per-session state and eventless or timed transitions. **Skills reach the agent that asked** — a skill now loads into the calling agent rather than being answered second-hand by a sub-agent, and discovery is local-first and offline. Plus three conversational front-door agents over the event subsystem, background LLM work as bounded single-flight tasks, `tool-args` as a plain JSON object, and `by run --serve` for a headless daemon. Opus remains the default model (`claude-code:opus` out of the box). Platform coverage: **macOS arm64** native binary plus a portable **JDK 21+ uberjar**; Linux and macOS amd64 binaries via the uberjar.
...
Pin a specific version with `BY_VERSION=v0.5.2` before piping to bash. See [`docs/install.md`](docs/install.md) for manual install, checksum verification, and troubleshooting.
