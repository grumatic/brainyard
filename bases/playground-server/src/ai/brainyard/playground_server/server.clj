...
(ns ai.brainyard.playground-server.server
  "http-kit lifecycle for the playground control plane (Phase-0 stub).

     cd bases/playground-server && clojure -M:run [port]   ; default 8090
  ...
  (:require [org.httpkit.server :as hk]
            [ai.brainyard.playground-server.routes :as routes]
...
(defn start!
  ...
  ([{:keys [port] :or {port 8090}}]
   ...
   (println (str "playground-server listening on http://localhost:" port
