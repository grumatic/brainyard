  ...
  (testing ":quiet display-format renders only the think text with a '●' bullet —
    ...
    (let [state {:iteration 3 :max-iterations 10 :stage :think
      ...
      (with-redefs [s/quiet? (constantly true)]
        ...
        ;; No think text yet → nothing to render (update-iteration-block! skips it).
        (is (= [] (#'s/render-iteration-block-lines
                   (dissoc state :reasoning :streaming) "⠋"))
            "empty when the iteration has no think text")
        ;; Streaming text takes precedence and still bullets.
        (is (str/includes?
